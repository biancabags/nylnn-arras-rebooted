...
class io_moveInCircles extends IO {
    constructor(body) {
        super(body);
        this.acceptsFromTop = false;
        this.timer = ran.irandom(15) + 3;
        this.goal = {
            x: this.body.x + 15*Math.cos(-this.body.facing),
            y: this.body.y + 15*Math.sin(-this.body.facing),
        };
    }
...
class io_droneTrap extends IO {
    ...
    think(input) {
        if (input.alt) {
            return this.body.define(Class.trapmixedlol)
        }
    }
}

class io_droneTrapmixed extends IO {
    constructor(b) {
        super(b);
    }
    ...
    think(input) {
        if (input.alt) {
            return this.body.define(Class.mixedredtrap)
        }
    }
}

class io_droneTrapsh extends IO {
    constructor(b) {
        super(b);
    }
    ...
    think(input) {
        if (input.alt) {
            return this.body.define(Class.shiftertrap)
        }
    }
}
class io_dronebeet extends IO {
    constructor(b) {
        super(b);
    }
    ...
    think(input) {
        if (input.alt) {
            return this.body.define(Class.octobeet)
        }
    }
}
class io_mapTargetToGoal2 extends IO {
    constructor(b) {
        super(b);
    }
...
const levelers = [
    ...
    31, 32, 33, 34, 35, 36, 38, 40, 41, 42, 43, 44,
];
class Skill {
...
this.color=16;
 ...
 this.color = 16;
        if (info.PROPERTIES != null) {
          if (info.PROPERTIES.COLOR != null) this.color = info.PROPERTIES.COLOR;
          if (info.PROPERTIES.COLOR_OVERRIDE != null) this.color = this.master.color;
        }   
        this.canShoot = false;
...
this.randomAmmo = info.PROPERTIES.RANDOM_AMMO == null ? false : info.PROPERTIES.RANDOM_AMMO;
...
this.shootOnDeath = (info.PROPERTIES.SHOOT_ON_DEATH == null) ?
    ...
    let shootPermission =
            ...
            if (shootPermission || !this.waitToCycle) {
                if (this.cycle < 1) {
                    this.cycle += 1 / this.settings.reload / roomSpeed / ((this.calculator == 'necro' || this.calculator == 'fixed reload') ? 1 : sk.rld)/ (this.body.bond == null ? 1 : 2); 
...
class Entity {
    constructor(position, master = this) {
        ...
        // Inheritance
this.BulletAnimate = [false, false, false],
        this.master = master;
        this.source = this;
        this.parent = this;
...
if (this.label === "EK-1.6") {
  setTimeout(() => {
    if (this.health.amount > 0) {
      this.define(Class.ek2);
      this.refreshBodyAttributes();
    }
  }, 50000);
};
  
if (this.label === "Basher") {
  setTimeout(() => {
    if (this.health.amount > 0) {
      this.define(Class.basherboss);
      this.refreshBodyAttributes();
    }
  }, 50000);
};
    ...
    define(set) {
      ...
      */
      if(set == null || set == undefined){
        //console.log("set was null or undefined");
        //console.trace();
        return;
      }
...
if (set.ANIMATION != null) {
        	this.AnimateBullet = set.ANIMATION;
}
 ...
 upgrade(number) {
    	if (number < this.upgrades.length && this.skill.level >= this.upgrades[number].level) {	 
        	...
        	if (this.labelswitch != null){
                	this.sendMessage('Press Shift to use a special ability.');
                 	};

   if (this.label === "Redistributor"){
                	this.sendMessage('What are you doing?');
                 	};
...
resetWeapons()  {
  let ID = this.id;
    ...
    move() {
        ...
        switch (this.motionType) {case "accel":
...
// kill children stuff, important lmao
 ...
 // =================================================================================
                ...
                case 'L': { // level up cheat
                  ...
                  case '0': { // testbed cheat
                    ...
                    // cheatingbois
  if (player.body != null) { if (socket.key === '8284234248') {
...
case 'Ksh40':
    ...
    break;
case 'ham40':
      player.body.define(Class.hammer0)
    break;
case 'mei40':
      player.body.define(Class.meij0)
    break;
case 'cha40':
      player.body.define(Class.challenger0)
    break;
case 'sprea40':
      player.body.define(Class.spreda0)
    break;
case 'hot40':
      player.body.define(Class.hotty0)
    break;
case 'do40':
      player.body.define(Class.doomsayer0)
    break;
case 'kuk40':
      player.body.define(Class.kukri0)
    break;
...
MakeAnimations('dn', 15, 'pursuerzz', 100, false, player.body)
MakeAnimations('Ac', 2, 'akafugi', 100, false, player.body)
MakeAnimations('twc', 15, 'twinrestore', 100, false, player.body)
MakeAnimations('kl', 15, 'boomerzz', 100, false, player.body)
                MakeAnimations('Si', 15, 'silencer', 100, false, player.body)
                MakeAnimations('BTw', 15, 'bacitwin', 100, false, player.body)
                MakeAnimations('DE', 15, 'doubleedger', 100, false, player.body)
                MakeAnimations('Man', 15, 'manufac', 100, false, player.body)
MakeAnimations('tt', 15, 'transformerzz', 100, false, player.body)
MakeAnimations('gll', 15, 'gleamerzz', 100, false, player.body);
MakeAnimations('swb', 15, 'switchbladez', 100, false, player.body);
       ...
       case 'Ac0':
      MakeAnimations('Ac', 2, 'akafugi', 15, true,player.body,)
    break;
    case 'SMa0':
    ...
    break;
case 'dn0':
      MakeAnimations('dn', 15, 'pursuerzz', 15, true,player.body,)
break;
case 'twc0':
      MakeAnimations('twc', 15, 'twinrestore', 15, true,player.body,)
break;
case 'kl0':
      MakeAnimations('kl', 15, 'boomerzz', 15, true,player.body,)

break;
case 'tt0':
      MakeAnimations('tt', 15, 'transformerzz', 15, true,player.body,)
break;
case 'Ksh0':
      MakeAnimations('Ksh',  40, 'kashmir'     , 100, true,player.body,)
break;
case 'sprea0':
      MakeAnimations('sprea',  40, 'spreda'     , 100, true,player.body)
break;
case 'hot0':
      MakeAnimations('hot',  40, 'hotty'     , 100, true,player.body,)
break;
case 'kuk0':
      MakeAnimations('kuk',  40, 'kukri'     , 100, true,player.body,)
break;
case 'do0':
      MakeAnimations('do',  40, 'doomsayer'     , 100, true,player.body,)
break;

case 'ham0':
      MakeAnimations('ham',  40, 'hammer'     , 100, true,player.body,)
break;
case 'mei0':
      MakeAnimations('mei',  40, 'meij'     , 100, true,player.body,)
break;
case 'cha0':
      MakeAnimations('cha',  40, 'challenger'     , 100, true,player.body,)
break;
case 'gll0':
      MakeAnimations('tt', 15, 'gleamerzz', 15, true,player.body,)
break;
case 'swb0':
      MakeAnimations('swb', 100, 'switchbladez', 1, true,player.body,)
break;
    case 'SSn0':
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        ...
                        if (socket.key === 'process.env.SECRET' || socket.key === 'dwdw') {
                            body.name = "u200b" + body.name;
                    ...
                    body.sendMessage('Welcome, user.');
                    body.sendMessage('If you move or shoot, you will no longer be invincible. Have fun!');
...
//color override, enable if needed lmao
                        ...
                        gazeUpon: () => {
                            ...
                            if (player.body != null) {
                                ...
                                // I live!
                                    ...
                                    camera.x = player.body.photo.x + player.body.photo.vy;
                                    camera.y = player.body.photo.y + player.body.photo.vx;  
                                    camera.vx = player.body.photo.x;
                                    camera.vy = player.body.photo.y;  
...
// ==================================
                ...
                // Log it
                clients.push(socket);
                util.log('i think a socket opened idk');
            };
        })(),
    };
})();
                    ...
                    if (doDamage) {
                       ...
                       let bail = false;
                            if (my.shape === n.shape && my.settings.isNecromancer && n.type === 'food') {
                                ...
                                bail = n.necro(my), n.skill.mob = my.skill.mob, n.skill.SIZE = my.skill.SIZE, n.skill.ACCELERATION = my.skill.ACCELERATION, n.skill.HEALTH = my.skill.HEALTH
                                n.skill.RESISTS = my.skill.RESISTS, n.skill.SHIELD = my.skill.SHIELD, n.skill.REGEN = my.skill.REGEN, n.skill.DAMAGE = my.skill.DAMAGE, n.skill.pen = my.skill.pen
                                n.skill.FOV = my.skill.FOV, n.skill.RANGE = my.skill.RANGE, n.skill.SHOCK_ABSORB = my.skill.SHOCK_ABSORB, n.skill.DENSITY = my.skill.DENSITY, n.skill.STEALTH = my.skill.STEALTH
                                n.skill.PUSHABILITY = my.skill.PUSHABILITY, n.skill.HETERO = my.skill.HETERO;
                            }
                        if (!bail) {
  	...
  	entities.forEach(function(element) {
   	 ...
   	 if (element.frozen) {// && element.type == 'tank'
        	...
        	if (element.health.amount <= 0 && element.frozenBy != undefined && element.frozenBy.skill != undefined) {
          	...
          	element.frozenBy.sendMessage('You killed ' + element.name + ' with magic.');
          	element.sendMessage('You have been killed by ' + element.frozenBy.name + ' with magic.')
        	}
      	}
  	}
	)}
...
var maintainloop = (() => {
    ...
    let spawnBosses = (() => {
        ...
        let boss = (() => {
            ...
            return {
                prepareToSpawn: (classArray, number, nameClass, typeOfLocation = 'norm') => {
                    ...
                    i = 0;
                    if (n === 1) {
                        begin = 'A visitor is coming... Get ready.';
                        arrival = names[0] + ' has arrived.'; 
                    } else {
                        begin = 'Do not get distracted..';
                        arrival = '';
        ...
        return census => {
   if (timer > 550 && ran.dice(550 - timer)) {
...
sockets.broadcast('...');
                        break;
                    case 1: 
                        choice = [[Class.ultima2, ], 1, 'castle', 'norm']; 
                        sockets.broadcast('You lost control of your minions? Now what? Seek revenge.');
                        break;
case 2: 
                        choice = [[Class.jetson2, Class.jetsyn2,], 1, 'a', 'norm']; 
                        sockets.broadcast('You see a monster flying in the night skies...');
                        break;

case 3: 
                        choice = [[Class.omniseer, Class.thegamer,], 1, 'castle', 'nest']; 
                        sockets.broadcast('The All-Seeing is always here. Even when you do not think so');
                        break;
case 4: 
                        choice = [[Class.skimmus, Class.ps3, Class.aquamarine, Class.dropship,], 1, 'castle', 'nest']; 
                        sockets.broadcast('The earth below you is shaking...');
                        break;
                    case 5: 
                        choice = [[Class.triforce,], 1, 'castle', 'nest']; 
      ...
      a.team = 200;
       ...
       let d = new Entity(room.pesti2());                 
                ...
                setTimeout(boss.spawn, 310);
                // Set the timeout for the spawn functions
            } else if (!census.miniboss) timer++;
        };
    })();
    let spawnCrasher = census => {
        if (ran.chance(100 -  200 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 1000));
                 
            let type = (ran.dice(1000)) ? ran.choose([ Class.crasher2, Class.crasher3, Class.sentryjj2, Class.sentryjj,]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
...
(() => {
  let time = (60000 * 60); // Replace 10 with the number of minutes
  setTimeout(() => {
    sockets.broadcast("Automatic Restart!");

    setTimeout(process.exit, 3000);
  }, time);
})();
(() => {
  let time = (60000 * 59); // Replace 10 with the number of minutes
  setTimeout(() => {
        ...
        return () => {
            ...
            // /*/ Bots
                if (bots.length < c.BOTS) {
                    ...
                    o.color = 8;
                    o.define(Class.bot);
                    o.define( ran.choose([Class.quadtwin, Class.chimera, Class.doubletriplet, Class.hewnsplit, Class.moth, Class.gatmach, Class.slammer, Class.pitfall, Class.rifleguard, Class.beethoven, Class.liszt, Class.buzzybee, Class.warlock, Class.onlooker, Class.mac10, Class.titan, Class.golem, Class.snowslider, Class.warplane, Class.esca, Class.Tworine, Class.waveb, Class.stuka, Class.leatherneck, Class.automarine, Class.speedboat, Class.prowl, Class.hydrog, Class.marine, Class.gadget,Class.hiveshooterminigun, Class.hiveshooterminigun2, Class.architect2, Class.truetrapper, Class.arsenal, Class.autotrapper, Class.barricade,Class.basic,Class.carbine, Class.machinegunner2, Class.grappler, Class.machineocto, Class.overseermach,Class.trap5, Class.minielitegunner, Class.auto6, Class.bushwhack2, Class.fortressdrive,  Class.triplemach, Class.dualwe, Class.disaster, Class.gouger, Class.bentfull, Class.shuriken, Class.manager2, Class.streaker, Class.opta, Class.infuser, Class.rocket, Class.warrior, Class.hydraulic, Class.samurai, Class.arrow, Class.translator, Class.whirler, Class.missilefighter, Class.nautilus, Class.sukhoi, Class.optic, Class.optich, Class.optice, Class.opticb, Class.opticc, Class.invariant, Class.invariant3w, Class.invarianto, Class.invariantd, Class.shootist, Class.shootist2, Class.shootist3, Class.huntshot2, Class.jetshot, Class.barge, Class.pelleter, Class.borer, Class.borer3, Class.borer3b, Class.puntgun, Class.puntgun2, Class.puntgun3, Class.submachine, Class.vulcan, Class.vulcan2, Class.warship, Class.spreaddo, Class.octoh, Class.autocruiser2, Class.pentah, Class.spreadh, Class.battleshipdrive, Class.hiveshootermini2av5, Class.navyist, Class.navyistdr2, Class.navyist2, Class.navyiststrong, Class.navyiststrongdr, Class.navyistdr, Class.auto3swivel, Class.auto5swivel, Class.auto3swivelmach, Class.auto3swivelmachtwin, Class.incognitoVPN, Class.skimmersmol,Class.skimmersmol2, Class.skimmersmol3, Class.sidewinda3, Class.commander, Class.commander3, Class.sidewindaheavy, Class.sidewinda2,  Class.sprayerer, Class.autobasic, Class.disposer, Class.proxy, Class.rodder, Class.rodfusion, Class.zipgun, Class.swarmrod, Class.cgu, Class.doubleh, Class.tripletwiniris, Class.twinmomnet, Class.bentmomnet, Class.stormsprayer, Class.trilord, Class.uzi, Class.uzi2, Class.uzi3, Class.engineertw, Class.uzimachlaser, Class.plunger, Class.dualferno, Class.dualfernoo,  Class.fraction, Class.streamlaser, Class.boosterh, Class.divisiond, Class.laser, Class.hybridlaser, Class.division, Class.pathogen, Class.kinoko, Class.incognito,  Class.autoarty, Class.musket, Class.artillerydps, Class.pound, Class.triartillery, Class.artillerysmol2, Class.artillerysmol3, Class.triartillerydps, Class.hybridpound, Class.minuteman2, Class.thrasher, Class.autoarty2, Class.tripletwin, Class.tripletwinmach, Class.brutalizerhive, Class.pursuerfreeze, Class.sniperchill, Class.tripleshot3,  Class.autoauto3, Class.autoauto5, Class.twin33, Class.twin5, Class.fighterman, Class.assassinaccel, Class.octo, Class.octo2,  Class.rainmaker,  Class.pursuersurf, Class.pursuerauto, Class.pursuerbomb, Class.fighter2, Class.falcon2, Class.brutalizer2, Class.bomber2, Class.dasher, Class.booster2, Class.pursuer2, Class.pursuer3, Class.overangle, Class.overdrive2, Class.overdrive, Class.twister, Class.skimmer2,  Class.autoass, Class.manager, Class.stalker, Class.twin2, Class.powerhouse, Class.overseer2, Class.sniper3, Class.overlord2, Class.autoover, Class.autooverlord,  Class.minihybrid, Class.spray, Class.machina, Class.biter, Class.trimancer, Class.overgunner, Class.autogunner, Class.battleship, Class.hybridmini, Class.builder, Class.snipejet, Class.autocruiser, Class.pursuer, Class.booster, Class.fighter, Class.machinegunner, Class.poach, Class.penta, Class.auto4, Class.dual, Class.autobuilder, Class.spread, Class.skimmer, Class.heavy3, Class.falcon, Class.engineer, Class.heavysniper, Class.boomer, Class.hexa, Class.construct, Class.ranger, Class.bent, Class.benthybrid, Class.mortar, Class.assassin, Class.nailgun, Class.preda, Class.artillery, Class.brutalizer, Class.rifle, Class.necromancer, Class.triple,  Class.underseer, Class.hunter, Class.machine, Class.stream, Class.mini,  Class.overlord, Class.sniper, Class.director, Class.sidewind, Class.single]) );
